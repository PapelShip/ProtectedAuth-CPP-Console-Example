...
int main() {
    ...
    std::string userKey;
    char* savedKey = qpapel::GetAutoLoginKey(ctx);
    if (savedKey != nullptr && strlen(savedKey) > 0) {
        userKey = savedKey;
        std::cout << "[Auto-Login] Saved key detected: " << userKey << std::endl;
        qpapel::FreeString(savedKey);
    } else {
        if (savedKey) qpapel::FreeString(savedKey);
    ...
    if (sessionToken != nullptr) {
        ...
        while (running) {
            ...
            std::cout << "5. List Available Products" << std::endl;
            std::cout << "6. Load Product (PE Loader)" << std::endl;
